...
ここでは教科書に合わせて原点を0にしていますが、散布図では原点を0にすることはあまり意味がないので、特に軸の範囲を指定しなくてもよいです。
...
他にも、スピアマンの順位相関係数、ケンドールの順位相関係数などがありますが、最もよく利用されるのがピアソンの積率相関係数なので、
...
| 用語法 | 結果を表す変数 | 原因を表す変数 |
|---|---|---|
| 一般的? | 従属変数 (dependent variable) | 独立変数 (independent variables) |
| 経済系? | 被説明変数 (explaned variable) | 説明変数 (explanatory variables)   |
| 統計系? | 目的変数 (objective variable) | 説明変数 (explanatory variables) |
| 機械学習? | 応答変数 (response variable) | 予測変数 (predictor variable) |
