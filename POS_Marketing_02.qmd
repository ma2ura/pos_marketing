...
```{r}
df_a <- df_chi2 |>
  pivot_longer(cols = -年代, names_to = "性別", values_to = "販売個数")
df_b <- df_chi_kitai |>
  pivot_longer(cols = -年代, names_to = "性別", values_to = "期待個数") |>
  mutate(性別 = substr(性別,1,2))
df_chi_test <- df_a |>
  left_join(df_b, by = c("年代", "性別"))
df_chi_test
```
...
このクロス集計表`df_chi2`は`data.frame`型ですが、`chisq.test()`関数は`matrix`型を引数として受け取るため、`as.matrix()`関数を使って`matrix`型に変換します。
年代を行、性別を列とする`matrix`型のクロス集計表を作成し、`chisq.test()`関数に渡します。

```{r}
O <- as.matrix(df_chi2[,2:3])
rownames(O) <- df_chi2$年代
E <- as.matrix(df_chi_kitai[,2:3])
rownames(O) <- df_chi2$年代
chisq.test(O, correct = FALSE)
```
