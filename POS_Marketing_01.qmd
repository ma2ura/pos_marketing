...
title: |
  ...
  <span style="color: #282A36; ">第1回 売上をまとめた表</span>
...
```{r}
pacman::p_load(tidyverse, readxl, tidyplots)
```
...
可視化はRの得意分野です。
最近話題の`tidyplots`パッケージを使って，いろんなグラフで表示してみましょう。
...
```{r}
df_maker_sales |>
  tidyplot(x = メーカー, y = 売上金額合計, color = メーカー) |>
  adjust_font(family = "HiraKakuProN-W3")
```
