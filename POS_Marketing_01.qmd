...
- `gganimate` : `ggplot2`で作成したグラフをアニメーションにするためのパッケージ
...
```{r excel_sheets, filename = "Excelファイルのシートの確認"}
...
```{r read_excel, filename = "Excelデータの読み込み"}
...
```{r check_str, filename = "データの属性を確認"}
str(df)
```

`dplyr::glimpse()`関数を使うと，データの概要をより見やすく表示できます。

```{r check_glimpse, filename = "データの概要を確認"}
dplyr::glimpse(df)
```
...
```{r summary_numeric, filename = "数値型の変数の記述統計量を確認"}
df |>
  select(where(is.double)) |> # 数値型の変数だけを選択
  summary() # 記述統計量を表示
```
...
```{r change_factor, filename = "文字型を因子型に変換"}
df <- df %>%
  mutate(
    曜日 = factor(
      曜日,
...
```{r recheck_summary, filename = "データの概要を再確認"}
summary(df)
```
...
```{r}
df |>
  dplyr::filter(メーカー == "自社") |> # 自社のデータだけを抽出
  head() # 最初の6行を表示
```
...
```{r maker_sales, filename = "メーカーごとの売上金額合計"}
...
```{r maker_sales_gt, filename = "メーカーごとの売上金額合計の表"}
df_maker_sales |>
  gt() |>
  fmt_number(columns = 2:2, decimals = 0) |> # 数値の書式を指定
  tab_header(title = "表1-2 メーカーごとの売上金額") |>
  gt_theme_pff() |> # テーマを適用
  as_raw_html() # HTML形式で出力
```

次に，各メーカーのどの商品が，平均何円で売られているのか，を調べてみましょう。
`summarise()`関数の引数に，`.by=c()`を指定し，グループ化したい変数として`メーカー`と`商品名`を指定してグループごとの平均単価を計算する。
そして`arrange()`関数でメーカーと平均単価でソートします。
`desc()`関数は降順にソートするための関数です。

```{r}
df_maker_item <- df |>
  summarise(
    .by = c(メーカー, 商品名), # メーカーと商品名でグループ化
    平均単価 = mean(単価) # 平均単価を計算
    ) |>
...
ここで注意しないといけないことは`summarise()`関数に`.by=`引数を使った場合，グループごとに1つの統計量を返すため，上のコードだと，
...
```{r month_time_sales_animation, filename ="月ごとの時間別売上高"}
#| cache: true
df_month_time_sales |>
  ggplot() +
...
```{r anime_month_time, filename = "時間ごとの月別売上高"}
#| cache: true
df_month_time_sales |>
  ggplot() +
...
テキストではMS Excelで作成した表を、MS Powerpointに貼り付けてスライド資料を作成していますが，
